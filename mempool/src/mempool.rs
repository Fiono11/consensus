use crate::batch_maker::{Batch, BatchMaker, NewBatch, NewTransaction, Transaction};
...
pub struct Mempool {
    ...
    /// Send messages to consensus.
    tx_new_consensus: Sender<Vec<Digest>>,
}
...
impl Mempool {
    ...
    fn handle_clients_transactions(&self) {
        ...
        let (tx_new_batch_maker, rx_new_batch_maker) = channel(CHANNEL_CAPACITY);
        let (tx_quorum_waiter, rx_quorum_waiter) = channel(CHANNEL_CAPACITY);
        let (tx_processor, rx_processor) = channel(CHANNEL_CAPACITY);
        let (tx_digests, rx_digests) = channel(CHANNEL_CAPACITY);
        ...
        NetworkReceiver::spawn(
            address,
            /* handler */ TxReceiverHandler { tx_batch_maker, tx_new_batch_maker },
        );
...
struct TxReceiverHandler {
    ...
    tx_new_batch_maker: Sender<NewTransaction>,
}
...
impl MessageHandler for TxReceiverHandler {
    async fn dispatch(&self, _writer: &mut Writer, message: Bytes) -> Result<(), Box<dyn Error>> {
        ...
        // Send the new_transaction to the batch maker.
        self.tx_new_batch_maker
            .send(NewTransaction(message.to_vec()))
            .await
            .expect("Failed to send transaction");
...
impl MessageHandler for MempoolReceiverHandler {
    async fn dispatch(&self, writer: &mut Writer, serialized: Bytes) -> Result<(), Box<dyn Error>> {
        ...
        match bincode::deserialize(&serialized) {
            Ok(MempoolMessage::NewBatch(batch)) => {
                info!("Received batch: {:?}", batch);
                for tx in &batch.0 {
                    info!("Received tx: {:?}", tx);
                }
                let digests = batch.0.iter().map(|tx| tx.digest()).collect();
                self
                    .tx_digests
                    .send(digests)
                    .await
